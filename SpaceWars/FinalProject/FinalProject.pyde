...
class SpaceShip:
    def __init__(self,x,y):
        ...
        self.keyHandler={LEFT:False, RIGHT:False, UP:False, SHIFT:False}  #dictionary to set values for keyboard buttons
...
class Powerups:
    def __init__(self):
        ...
        self.bullet = loadImage(path + '/images/3bullets.png')
        self.bullet3list = []
    ...
    #     if t.time%200==0:
    #         self.x = 10 * randint(0,60)
    #         self.bullet3list.append([self.x,self.y])
    #     for i in range(len(self.bullet3list)):
    #         image(self.bullet,self.bullet3list[i][0],self.bullet3list[i][1],100,100)
    #         self.bullet3list[i][1]+=10
    #         if self.bullet3list[i][0] in range(t.s.x-70,t.s.x+120) and self.bullet3list[i][1] in range(875,925):
    #             self.pole = True
    #             self.bullet3list[i][1] = 1000
    #         if self.pole == True:
    #             if self.timeB3 <= 150:
    #                     self.timeB3 +=1
    #             else:
...
class Game:
    ...
    def display(self):
        if self.state == 'initial': #to display logo
            ...
            if self.time>=900 and (stage ==1 or stage == 2): #bring boss after 900 frames for stage 1 and 2
                ...
                stroke(255)
                noFill()
                rect(4, 20, 101, 20)#display the health bar
                noStroke()
                fill(255,0,0)
                rect(5, 21, self.bhealth, 18)
    ...
    def healthpointer(self):
        ...
        stroke(255)
        noFill()
        rect(578, 20, 101, 20)#display the health bar
        noStroke()
        fill(0,255,0)
        rect(579, 21, self.health, 18)
